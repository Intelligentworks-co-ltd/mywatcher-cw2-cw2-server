...
** that does not care about crash recovery or rollback_transaction might make
...
** rollback_transaction, ABORT processing, DROP TABLE, or by any other
...
** forces an automatic rollback_transaction of the changes.  See the
...
** explicit transaction then you should rollback_transaction the transaction before
** continuing.
**
...
** to the callback.  If the callback on a commit_transaction hook function 
** returns non-zero, then the commit_transaction is converted into a rollback_transaction.
**
...
** an error or constraint causes an implicit rollback_transaction to occur.
** The rollback_transaction callback is not invoked if a transaction is
** automatically rolled back because the database connection is closed.
** The rollback_transaction callback is not invoked if a transaction is
** rolled back because a commit_transaction callback returned non-zero.
...
**          then the commit_transaction hook callback is cancelled and no callback
**          is invoked when a transaction commits.
**
** {F12955} If the commit_transaction callback returns non-zero then the commit_transaction is
**          converted into a rollback_transaction.
**
...
**          then the rollback_transaction hook callback is cancelled and no callback
...
**
** Closing a BLOB shall cause the current transaction to commit_transaction
...
**          commit_transaction if there are no other open [sqlite3_blob] objects
